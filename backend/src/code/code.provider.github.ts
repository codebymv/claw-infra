...
export class CodeProviderGithub {
  ...
  private async requestJson<T>(url: string): Promise<T> {
    ...
    if (!response.ok) {
      ...
      this.logger.warn(
        `GitHub API request failed (${response.status}) ${url}: ${text}`,
      );
  ...
  private normalizeReviewState(input: unknown): CodePrReviewState {
    const stateRaw =
      typeof input === 'string' ? input.toUpperCase() : 'COMMENTED';

    if (stateRaw === CodePrReviewState.APPROVED)
      return CodePrReviewState.APPROVED;
    if (stateRaw === CodePrReviewState.CHANGES_REQUESTED)
      return CodePrReviewState.CHANGES_REQUESTED;
    if (stateRaw === CodePrReviewState.COMMENTED)
      return CodePrReviewState.COMMENTED;
    if (stateRaw === CodePrReviewState.DISMISSED)
      return CodePrReviewState.DISMISSED;
    if (stateRaw === CodePrReviewState.PENDING)
      return CodePrReviewState.PENDING;
  ...
  async fetchPullRequests(
    ...
    for (let page = 1; page <= 10; page += 1) {
      ...
      const pageItems = await this.requestJson<GithubListResponseItem[]>(
        url.toString(),
      );
      ...
      for (const row of pageItems) {
        ...
        const authorLogin =
          ...
          typeof (row.user as { login?: unknown }).login === 'string'
        ...
        if (prNumber > 0) {
          try {
            ...
            const detail =
              await this.requestJson<GithubListResponseItem>(detailUrl);
            additions =
            ...
            this.logger.warn(
              `Failed to fetch details for PR #${prNumber}: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        }
        ...
        items.push({
          externalId: String(
            row.id ?? `${repo.owner}/${repo.name}#${String(row.number ?? '')}`,
          ),
          number: prNumber,
  ...
  async fetchPullRequestReviews(
    repo: GithubRepoRef,
    prNumber: number,
  ): Promise<GithubPullRequestReviewRecord[]> {
    ...
    for (let page = 1; page <= 10; page += 1) {
      ...
      for (const row of pageItems) {
        ...
        items.push({
          externalId: String(
            row.id ?? `${repo.owner}/${repo.name}#${prNumber}:${items.length}`,
          ),
    ...
    for (let page = 1; page <= 10; page += 1) {
      ...
      for (const row of pageItems) {
        ...
        const committedAtRaw =
          ...
          typeof (detail.commit as { author?: { date?: unknown } }).author
            ?.date === 'string'
            ? ((detail.commit as { author: { date: string } }).author.date ??
              null)
            : null;
        ...
        const commitAuthor =
          ...
          typeof detail.commit === 'object' &&
          (detail.commit as { author?: unknown }).author &&
          typeof (detail.commit as { author?: { name?: unknown } }).author
            ?.name === 'string'
            ? ((detail.commit as { author: { name: string } }).author.name ??
              null)
            : null;
        ...
        commits.push({
          ...
          additions:
            ...
            typeof (detail.stats as { additions?: unknown }).additions ===
              'number'
          ...
          deletions:
            detail.stats &&
            typeof detail.stats === 'object' &&
            typeof (detail.stats as { deletions?: unknown }).deletions ===
              'number'
  ...
  async handleWebhookEvent(
    input: GithubWebhookHandleInput,
      ...
      if (fullName && fullName.includes('/')) {
        ...
        const owner =
          ...
          typeof payload.repository.owner.login === 'string'
            ...
            : null;
        const name =
          payload.repository && typeof payload.repository.name === 'string'
    ...
    return {
      ...
      note:
        repos.length > 0
          ? undefined
          : 'Webhook accepted but no repository context was found',
    };
  }
}
